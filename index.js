...
function fenomenEkle(fenomen, number, profile, followers, post, platform) {
  const copyArr = [...fenomen];
  const newArr = {
    "number": number,
    "profile": profile,
    "followers": followers,
...
function platformaGoreCokGonderiYapanFenomen(fenomen, platform){
  ...
  for (let i = 0; i < fenomen.length; i++) {
    if (fenomen[i].platform === platform) {
      if (fenomen[i].posts > maxPost) {
        maxPost = fenomen[i].posts
        name = fenomen[i].profile
      }
    }
  }
  return name
